<template>
	<scroll-view style="height: 100%" scroll-y class="orders" @scrolltolower="onScrolltolower">
		<view class="card" v-for="order in list" :key="order.id">
			...
			<view class="action">
				...
				<template v-else>
					<navigator class="button secondary" :url="`/pagesOrder/create/index?orderId=${order.id}`" hover-class="none">
						再次购买
					</navigator>
					<!-- 待收货状态: 展示确认收货 -->
...
import { getMemberOrderListAPI } from '@/api/order';
...
// 获取订单列表数据
const params = { orderState: props.orderState };
const { list, page_params, finish, getPageListData } = useGetPageList<OrderItem, typeof params>(
	getMemberOrderListAPI,
	params
);
page_params.pageSize = 5;
// 滚动到底部时触发
